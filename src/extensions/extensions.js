...
function estimatePrepTime(array, prepTime = 2) {
  return array.length * prepTime
}
...
function calculateQuantities(ingredients, layers) {
  return ingredients.reduce(
    (quantities, ingredient) => {
      if (ingredient === 'sugar') {
        quantities.sugar = 100 * layers
      }
      if (ingredient === 'eggs') {
        quantities.eggs = 2 * layers
      }
      return quantities
    },
    { sugar: 0, eggs: 0 }
...
function improveRecipe(ingredients, portions) {
  ...
  for (const ingredient in ingredients) {
    improvedRecipe[ingredient] = ingredients[ingredient] * portions
  }
